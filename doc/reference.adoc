...
Usage: shmate-artifact <task> [<task_arg> ...]
...
Handler called just before <<user-content-src_lib_shmate_base-shmate_cleanup>>, but not upon receiving signal. It should never be called manually, but can be implemented in caller's script.
...
'''
[#user-content-src_lib_shmate_job-shmate_run_qualified_job,reftext='shmate_run_qualified_job']
...
'''
[#user-content-src_lib_shmate_job-shmate_run_qualified_muted_job,reftext='shmate_run_qualified_muted_job']
